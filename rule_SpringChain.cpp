...
void Rule_SpringChain::Init(Vector2 topPos, Vector2 bottomPos, float chainSplit, float chainWidth)
{
	...
	//リストをクリアしておく
	m_RuleChainPointList.clear();

	float swingPower = 1.0f;

	//点のリスト初期化
	for (int i = 0; i < m_ChainPointCount; i++)
	{
		Rule_ChainPoint pointList;
		...
		m_RuleChainPointList.push_back(pointList);
	}
...
void Rule_SpringChain::Uninit()
{
	UnInitSprite();
}

void Rule_SpringChain::Update()
{
	//重力適用
	for (auto& point : m_RuleChainPointList)
	{
		if (!point.lock)
		{
	...
	for (int n = 0; n < constraintLoop; n++)
	{
		...
		{
			Rule_ChainPoint& point1 = m_RuleChainPointList[i];
			Rule_ChainPoint& point2 = m_RuleChainPointList[i + 1];
	...
	if (m_RuleChainPointList.front().pos.y > -screenHeight / 4)
	{
		//一番上の点を固定
		m_RuleChainPointList.front().lock = true;
	}
}

void Rule_SpringChain::Draw()
{
	for (int i = 0; i < m_ChainPointCount - 1; i++)
	{
		DrawPiece(i);
...
void Rule_SpringChain::DrawPiece(int count)
{
	Rule_ChainPoint point1 = m_RuleChainPointList[count];
	Rule_ChainPoint point2 = m_RuleChainPointList[count + 1];
